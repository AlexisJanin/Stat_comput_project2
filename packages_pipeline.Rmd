...
X_eval = 0:n/(n/5)      #X_eval, we accorded on 100 or 1000 or 5000 points; shouldn't matter                                        much!
Y <- log_sin(X,a=3)            #Simply change de the function for the one wanted
Y_eval <- log_sin(X_eval,a=3)
...
y_locpoly <- locpoly(x = X,y = Y_noisy, bandwidth=h, degree=1,gridsize = n)

lf <- locfit(Y_noisy~lp(X,h=2*h/5,deg=1), data=my_data,kern = "gauss")  #it seems like since h is the halfradius,
preds <- predict(lf, newdata=my_data, se.fit=TRUE)$fit

plot(X,Y_noisy,col = "red")
lines(y_locpoly)
