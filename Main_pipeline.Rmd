...
degrees <- 1:5; len_d <- length(degrees)                              #Degree(s)
h_ <- (1.5:5.5)/10; len_h <- length(h_)                            #Bandwidth(s)
N_xeval <- 1000                           #Sizes of sample

Samples_sizes <- c(100,500,1000,2500,4500); s <- length(Samples_sizes) #length(xeval) < 5000 in locpol that's why 3 here atm

n_method = 3
n_trials = 1 #nb of different trials to record
n_trials_in = 5  #Nb of different trials but inside, and then mean taken
nb_functions = 1
  ...
  for (h in h_){
    for (n in Samples_sizes){
      ...
      for (q in 1:n_trials_in){
        ...
        Y <- dmixnorm(X, mu1 = 1, mu2 = 3, sigma1 = 1, sigma2 = 0.5, tau = 0.7)            #Simply change de the function for the one wanted
        Y_eval <- dmixnorm(X_eval, mu1 = 1, mu2 = 3, sigma1 = 1, sigma2 = 0.5, tau = 0.7)
        Y_noisy <- Noise_induction(Y,sig_gauss = abs(max(Y)-min(Y))/20)  #Variance of gaussian noise linearly correlated with TV(total variation of a function) so the noise is not too low or too big
      
        t_e_locpol <- err_time_comput(degree = d, bandwidth = h, X = X, Y_noisy = Y_noisy, X_eval = X_eval, Y_eval = Y_eval, method = "locpol")
        rem_pol <- rem_pol + c(d,h,n,t_e_locpol[1],t_e_locpol[2])
        
        t_e_locpoly <- err_time_comput(degree = d, bandwidth = h, X = X, Y_noisy = Y_noisy, X_eval = X_eval, Y_eval = Y_eval, method = "locpoly")
        rem_poly <- rem_poly + c(d,h,n,t_e_locpoly[1],t_e_locpoly[2])
        
        t_e_locfit <- err_time_comput(degree = d, bandwidth = h, X = X, Y_noisy = Y_noisy, X_eval = X_eval, Y_eval = Y_eval, method = "locfit")
        rem_fit <- rem_fit + c(d,h,n,t_e_locfit[1],t_e_locfit[2])
      }
      rem_pol = rem_pol/n_trials_in
      df_results[i,c(2:4,7:8)] <- rem_pol
      df_results[i,5:6] <- c("locpol","dmixnorm")
      i<-i+1
      remp_poly = rem_poly/n_trials_in
      df_results[i,c(2:4,7:8)] <- rem_poly
      df_results[i,5:6] <- c("locpoly","dmixnorm")
      i<-i+1
      rem_fit = rem_fit/n_trials_in
      df_results[i,c(2:4,7:8)] <- rem_fit
      df_results[i,5:6] <- c("locfit","dmixnorm")
      i<-i+1
    }
  }
}
