...
h_ <- (2:10)/10; len_h <- length(h_)                            #Bandwidth(s)
N_xeval <- 1000                           #Sizes of sample

Samples_sizes <- c(100,1000,4000); s <- length(Samples_sizes) #length(xeval) < 5000 in locpol that's why 3 here atm
...
for (d in degrees){
  for (h in h_){
    for (n in Samples_sizes){
      ...
      df_results[i,c(2:4,7:8)] <- c(d,h,n,t_e_locpoly[1],t_e_locpoly[2])
      ...
      df_results[i,c(2:4,7:8)] <- c(d,h,n,t_e_locfit[1],t_e_locfit[2])
