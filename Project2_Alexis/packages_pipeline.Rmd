...
errorlocfit <- 0
for (k in 1:K){
  lf <- locfit(accel~lp(times,h=h), data=mcycle[-ind[,k],], deg=d,kern = "gauss")
  preds <- predict(lf, newdata=mcycle[ind[,k],], se.fit=TRUE)$fit
  errorlocfit <- errorlocfit + sum(abs(mcycle$accel[ind[,k]] - preds))/k
}
errorlocfit/K

lf <- locfit(accel~lp(times,h=h), data=mcycle, deg=d,kern = "gauss")
...
```{r}
#Clear degree, clear bandwidth and UNclear/no kernel.
errorkern = 0
for (k in 1:K){
...
y_locpoly <- locpoly(x = mcycle$times,y = mcycle$accel, bandwidth=h, degree=2)
